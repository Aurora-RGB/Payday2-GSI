...
function GSICommunicator:SendMessage()
    local message_data = {
        provider = table.merge(self.ProviderDetails, {local_peer_id = managers.network:session() and managers.network:session():local_peer():id() or -1})
    }
    ...
    for _, inter in pairs(GSI.Intergrations) do
        if not inter.unreach_t or self._tick - inter.unreach_t >= self.Timeout then
            ...
            inter.uri, "/gameState/pd2", {}, tostring(encoded))
        end
    end
